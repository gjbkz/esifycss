...
export interface IHashOptions {
    algorithm: string,
    encoding: HexBase64Latin1Encoding | 'safe-base64' | typeof Buffer,
}

export const getHash = (
    data: Parameters<Hash['update']>[0],
    {algorithm, encoding}: IHashOptions = {algorithm: 'sha256', encoding: 'safe-base64'},
): string | Buffer => {
    const hash = createHash(algorithm);
    hash.update(data);
    if (encoding === 'safe-base64') {
