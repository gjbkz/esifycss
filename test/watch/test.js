...
t.test('watch', {timeout: 5000}, (t) => {
    ...
    t.test('watcher', {timeout: 3000}, async (t) => {
        ...
        t.match(exported.classes, {foo: '_foo_css_foo'}, 'classes');
        t.match(exported.properties, {color: 'gold'}, 'properties');
        t.match(exported.default, {
            classes: {name: 'classes'},
            properties: {name: 'properties'},
        }, 'default');
        const outputCSS = await afs.readFile(dest, 'utf8');
        const cssAST = postcss.parse(outputCSS);
        t.equal(cssAST.nodes[0].selector, `.${exported.classes.foo}`, 'cssAST.nodes[0].selector');
        t.match(cssAST.nodes[0].nodes[0], {
            prop: '--color',
            value: exported.properties.color,
        }, 'cssAST.nodes[0].nodes[0]');
    });
    t.end();
});
