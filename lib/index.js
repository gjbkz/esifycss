const path = require('path');
const console = require('console');
const chokidar = require('chokidar');
const minimatch = require('minimatch');
const {promisify} = require('util');
const glob = promisify(require('glob'));
const afs = require('@nlib/afs');
const postcss = require('postcss');

const isIgnored = (file, excludePatterns) => 0 < excludePatterns.length && excludePatterns.every((pattern) => minimatch(file, pattern));

const listFiles = async ({include, exclude}) => {
    const options = {
        nodir: true,
        ignore: '**/node_modules/**',
    };
    const results = await Promise.all(include.map((pattern) => glob(pattern, options)));
    const concatenated = [].concat(...results);
    return concatenated.filter((file) => !isIgnored(file, exclude));
};

class Cache extends Map {}

class Labeler extends Map {

    label(value) {
        if (!this.has(value)) {
            this.set(value, {label: this.size, count: 0});
        }
        const item = this.get(value);
        item.count += 1;
        return item.label;
    }

}

const getCSSAST = async (file, css, config) => {
    const processOptions = {from: file, ...config.processOptions};
    if (config.plugins) {
        return (await postcss(config.plugins).process(css, processOptions)).root;
    } else {
        return postcss.parse(css, processOptions);
    }
};

const getDependencies = (ast, id) => {
    const directory = path.dirname(id);
    const dependencies = new Map();
    let count = 0;
    ast.walkAtRules((node) => {
        if (node.name !== 'import') {
            return;
        }
        node.params.trim()
        .replace(/^(['"])([^\s'"]+)\1\s*([^\s]*)$/, (match, quote, target, givenName) => {
            count += 1;
            const name = givenName || `$${count}`;
            dependencies.set(name, afs.absolutify(target, directory));
            node.remove();
        });
    });
    return dependencies;
};

const minify = (ast) => {
    const {raws} = ast;
    for (const key of ['before', 'between', 'after']) {
        const value = raws[key];
        if (value) {
            raws[key] = value.replace(/\s/g, '');
        }
    }
    return ast;
};

const processCSSAST = ({file, ast, dependencies}, config) => {
    const classes = {};
    const properties = {};
    ast.walk((node) => {
        minify(node);
        if (node.type === 'rule') {
            node.selector = node.selector.replace(/([$_a-zA-Z0-9-]*)?\.=?(-?[_a-zA-Z]+[_a-zA-Z0-9-]*)/g, (match, imported, className) => {
                if (imported && dependencies.has(imported)) {
                    const sourceId = dependencies.get(imported);
                    const {classes: sourceClasses} = config.cache.get(sourceId);
                    return `.${sourceClasses[className]}`;
                } else {
                    if (!classes[className]) {
                        classes[className] = config.mangler(file, className);
                    }
                    return `.${classes[className]}`;
                }
            });
            node.raws.before = '\n';
        } else if (node.type === 'decl' && node.prop.startsWith('--')) {
            const propertyName = node.prop.slice(2);
            properties[propertyName] = node.value;
        }
    });
    return {classes, properties};
};

const processCSS = async (filePath, config) => {
    const file = path.normalize(filePath);
    const css = await afs.readFile(file, 'utf8');
    const hash = afs.getHash(css);
    const cached = config.cache.get(file);
    if (cached && hash === cached.hash) {
        return cached;
    }
    const ast = await getCSSAST(file, css, config);
    const dependencies = getDependencies(ast, file);
    for (const [, file] of dependencies) {
        await processCSS(file, config);
    }
    const {classes, properties} = processCSSAST({file, ast, dependencies}, config);
    if (config.onParse) {
        await config.onParse({
            file: path.normalize(file),
            classes: Object.assign({}, classes),
            properties: Object.assign({}, properties),
        });
    }
    const result = {file, hash, ast, classes, dependencies, properties};
    let destDir;
    if (config.outputDir) {
        const baseDir = config.baseDir || process.cwd();
        destDir = path.join(config.outputDir, path.relative(baseDir, path.dirname(file)));
    } else {
        destDir = path.dirname(file);
    }
    const filename = `${path.basename(file, path.extname(file))}${config.ext}`;
    let dest = path.join(destDir, filename);
    if (config.rename) {
        dest = config.rename(file);
    }
    afs.updateFile(dest, config.generateDefinition(result));
    config.cache.set(file, result);
    return result;
};

// options.mangler
// options.mangle
// options.labeler
// options.base
const getMangler = (options) => {
    let {mangler} = options;
    if (!mangler) {
        if (options.mangle) {
            const labeler = options.labeler || new Labeler();
            mangler = (file, className) => `_${labeler.label(`${file}/${className}`).toString(36)}`;
        } else {
            const base = options.base || options.baseDir || process.cwd();
            mangler = (file, className) => [
                path.relative(base, file).replace(/^(\w)/, '_$1').replace(/[^\w]+/g, '_'),
                className,
            ].join('_');
        }
    }
    return mangler;
};

const getGenerator = (options) => {
    if (options.generator) {
        return options.generator;
    } else if (options.classesOnly) {
        return ({classes}) => [
            `export const classes = ${JSON.stringify(classes)};`,
            'export default classes;',
        ].join('\n');
    } else {
        return ({classes, properties}) => [
            `export const classes = ${JSON.stringify(classes)};`,
            `export const properties = ${JSON.stringify(properties)};`,
            'export default {classes, properties};',
        ].join('\n');
    }
};

const concat = ({cache}) => [...cache].map(([, {ast}]) => ast.toString()).join('\n');

const getConfig = async (options) => {
    if (options.include && options.include.length === 0) {
        delete options.include;
    }
    const configFile = options.config ? afs.absolutify(options.config) : await afs.findUp('esifycss.config.js');
    const config = Object.assign(
        {ext: options.typescript ? '.css.ts' : '.css.js'},
        configFile ? require(configFile) : {},
        options,
    );
    config.config = configFile;
    config.cache = new Cache();
    config.mangler = getMangler(config);
    config.generateDefinition = getGenerator(config);
    const [include, exclude] = [config.include, config.exclude].map(
        (patterns) => [].concat(patterns).filter(Boolean).map((x) => x.split(path.sep).join('/')),
    );
    if (include.length === 0) {
        include.push('**/*.css');
    }
    config.include = include;
    config.exclude = exclude;
    config.files = await listFiles(config);
    return config;
};

const start = async (options) => {
    const config = await getConfig(options);
    await Promise.all(config.files.map((file) => processCSS(file, config)));
    if (config.dest) {
        await afs.updateFile(config.dest, concat(config));
    }
    return config;
};

const watch = async (options) => {
    const config = await start(options);
    const watcher = chokidar.watch(config.include, Object.assign({}, config.chokidar));
    const output = config.dest ? () => {
        clearTimeout(output.timer);
        output.timer = setTimeout(() => {
            afs.updateFile(config.dest, concat(config))
            .then(() => {
                watcher.emit('esifycss:output');
            })
            .catch(console.error);
        }, 200);
    } : () => {};
    const handle = (file) => {
        if (isIgnored(file, config.exclude)) {
            return;
        }
        processCSS(file, config)
        .then(() => {
            watcher.emit('esifycss:file', file);
            output();
        })
        .catch((error) => {
            watcher.emit('error', error);
            console.error(error);
        });
    };
    watcher
    .on('add', handle)
    .on('change', handle);
    watcher.esifycssConfig = config;
    return watcher;
};

Object.assign(exports, {
    listFiles,
    getConfig,
    start,
    watch,
    Cache,
});
